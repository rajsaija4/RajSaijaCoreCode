...
@objc open class ACFloatingTextfield: UITextField {
    ...
    @IBInspectable open var floatingPlaceHolderColor : UIColor = UIColor.lightGray {
        didSet{
            self.floatTheLabel()
        }
    }
    ...
    @IBInspectable open var selectedFloatingPlaceHolderColor : UIColor = UIColor(red: 19/256.0, green: 141/256.0, blue: 117/256.0, alpha: 1.0){
        didSet{
            self.floatTheLabel()
        }
    }
...
fileprivate extension ACFloatingTextfield {
    ...
    func addFloatingLabel() {
        ...
        labelPlaceholder?.textColor = self.floatingPlaceHolderColor
        labelPlaceholder?.font = UIFont.init(name: Constants.Font.YUGOTHIC_UI_SEMIBOLD, size: 15.0) //UIFont.init(name: (self.font?.fontName ?? "helvetica")!, size: 12)
        labelPlaceholder?.isHidden = true
        labelPlaceholder?.sizeToFit()
        labelPlaceholder?.translatesAutoresizingMaskIntoConstraints = false
        self.attributedPlaceholder = NSAttributedString(string: placeholder ?? "", attributes: [NSAttributedString.Key.foregroundColor: self.floatingPlaceHolderColor])
    ...
    func addErrorPlaceholderLabel() -> Void {
        ...
        labelErrorPlaceholder?.font = UIFont.init(name: Constants.Font.YUGOTHIC_UI_SEMIBOLD, size: 15.0) //UIFont(name: (self.font?.fontName ?? "helvetica")!, size: 12)
    ...
    func floatPlaceHolder(selected:Bool) -> Void {
        ...
        if selected {
            ...
            bottomLineViewHeight?.constant = 2;
            labelPlaceholder?.textColor = self.selectedFloatingPlaceHolderColor;
            self.attributedPlaceholder = NSAttributedString(string: placeholder ?? "", attributes: [NSAttributedString.Key.foregroundColor: self.selectedFloatingPlaceHolderColor])
        } else {
            bottomLineView?.backgroundColor = showingError ? self.errorLineColor : self.lineColor;
            bottomLineViewHeight?.constant = 1;
            self.labelPlaceholder?.textColor = self.floatingPlaceHolderColor
            self.attributedPlaceholder = NSAttributedString(string: placeholder ?? "", attributes: [NSAttributedString.Key.foregroundColor: self.floatingPlaceHolderColor])
        }
