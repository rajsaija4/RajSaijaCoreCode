...
class BuySellVC: BaseVC {
    ...
    @IBOutlet weak var viewFractionalShareBuy: UIView!
    @IBOutlet weak var lblFractionalShareBuy: UILabel!
...
//        NotificationCenter.default.removeObserver(self)
    ...
    func SetupAllSocketNotification() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(BuySellVC.getBuySellTradesData(notification:)), name: NSNotification.Name(kUpdateBuySellTrades), object: nil)
    }
    
    @objc func getBuySellBarData(notification: Notification) {
        debugPrint("=====Getting BuySell Bar Data=====")
...
//                self.objStockDetail.closePrice = ("\(barObject["closePrice"] ?? "")" as NSString).doubleValue
    ...
    func setupViewDetail() {
        DispatchQueue.main.async {
            ...
            self.viewFractionalShareBuy.createButtonShadow(BorderColor: UIColor.init(hex: 0x000000, a: 0.2), ShadowColor: UIColor.init(hex: 0x000000, a: 0.2))
        ...
        self.lblFractionalShareBuy.textColor = UIColor.labelTextColor
        ...
        if self.objStockDetail.fractionable == true {
            self.viewFractionalShareBuy.isHidden = false
            self.viewFractionalShareSell.isHidden = false
        } else {
            self.viewFractionalShareBuy.isHidden = true
        ...
        self.lblBuyingPower.text = "You have available only '$\(convertThousand(value: Double(self.buyingPower.clean) ?? 0.0))' buying power to buy stock"
...
//        }
    ...
    //SET LIVE DATA
    func setupLiveData() {
        self.lblStockPriceBuy.text = "$" + convertThousand(value: self.objStockDetail.current_price)
        self.lblStockPriceSell.text = "$" + convertThousand(value: self.objStockDetail.current_price)
        
        var strVariation: String = ""
        
        if self.objStockDetail.plVariationPer > 0 {
            strVariation = "$+\(convertThousand(value: self.objStockDetail.plVariationValue)) (+\(String(format: "%.2f", self.objStockDetail.plVariationPer))%)"
        } else if self.objStockDetail.plVariationPer < 0 {
            strVariation = "$\(convertThousand(value: self.objStockDetail.plVariationValue)) (\(String(format: "%.2f", self.objStockDetail.plVariationPer))%)"
        } else {
            strVariation = "$\(convertThousand(value: self.objStockDetail.plVariationValue)) (0.00%)"
        }
...
//                    }
    ...
    @IBAction func btnBuyFractionalShareClick(_ sender: UIButton) {
        if self.txtValidityBuy.text != "day" {
            GlobalData.shared.showSystemToastMesage(message: "Please select validity 'day' for fractional orders")
        } else if self.txtOrderTypeBuy.text != "market" {
